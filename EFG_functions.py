# In this file, the funcions that calculate the EFG are defined.

import numpy as np
from Crystalline_structure import base
...
# 'lattice_p' refers to the lattice parameter, 'pos' to the position in which the user
...
def point_charge_EFG(lattice, pos, s=15):

    a1 = np.array([lattice, 0, 0])
    a2 = np.array([0, lattice, 0])
    a3 = np.array([0, 0, lattice])
...
# This function diagonalize the EFG in the Principal Axis System (PAS) of the tensor, and finds
# the quadrupolar constant
def diagonalize_EFG(tensor, quadrupole_moment):
    ...
    Vxx = -(Vzz+Vyy)
    print(Vzz,Vxx,Vyy)
    eta = (Vyy - Vxx) / Vzz # Asymmetry parameter of the EFG tensor
